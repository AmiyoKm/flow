    ...
    <div className="flex w-full h-full">
      <aside className="w-[440px] min-w-[440px] max-w-[440px] border-r-2 border-separate flex flex-grow flex-col overflow-hidden">
        <div className="py-4 px-2">
          ...
          <ExecutionLabel value={duration ? duration : <Loader2Icon className="animate-spin" />} label="Duration" icon={ClockIcon} />
          <ExecutionLabel value={<ReactCountUpWrapper value={creditsConsumed} />} label="Credits consumed" icon={CoinsIcon} />
        </div>
        <Separator />
        <div className="flex justify-center items-center py-2 px-4">
            <div className="text-muted-foreground flex items-center gap-2">
                <WorkflowIcon size={30} className="stroke-muted-foreground/80" />
      ...
      <div className="flex w-full h-full">{ isRunning && (
...
function LogViewer({logs} : {logs : ExecutionLog[] | undefined}){
  ...
  return(
    ...
    <CardHeader className="rounded-lg rounded-b-none border-b py-4 bg-gray-50 dark:bg-background">
      ...
      <CardDescription className="text-muted-foreground">Logs generated by this phase</CardDescription>
    </CardHeader>
        ...
        <TableBody>
          {
            logs.map(log=>(
              <TableRow key={log.id} className="text-muted-foreground">
                ...
                <TableCell width={80} className={cn("uppercase text-xs font-bold p-[3px]" ,log.logLevel as LogLevel ==="error" && "text-destructive",
